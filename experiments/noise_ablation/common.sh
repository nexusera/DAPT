...
if [[ -n "${NOISE_ABLATION_COMMON_SH_LOADED:-}" ]]; then
  return 0
fi
NOISE_ABLATION_COMMON_SH_LOADED=1

set -euo pipefail

NOISE_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_DAPT_ROOT="/data/ocean/DAPT"
if [[ ! -d "$DEFAULT_DAPT_ROOT" ]]; then
  DEFAULT_DAPT_ROOT="$(cd "${NOISE_SCRIPT_DIR}/../.." && pwd)"
fi
...
gen_kv_config() {
  local variant="$1"
  local model_dir="$2"
  local out_cfg="${GEN_DIR}/kv_ner_config_noise_${variant}.json"

  "$PYTHON_BIN" - "$DAPT_ROOT" "$variant" "$REAL_TRAIN_JSON" "$REAL_TEST_JSON" "$model_dir" "$out_cfg" <<'PY'
import json
import sys
...
tpl = root / "dapt_eval_package" / "pre_struct" / "kv_ner" / f"kv_ner_config_noise_{variant}.json"
if not tpl.is_file():
...
gen_ebqa_config() {
  ...
  local ebqa_train="$3"
  local ebqa_eval="$4"
  local out_cfg="${GEN_DIR}/ebqa_config_noise_${variant}.json"

  "$PYTHON_BIN" - "$DAPT_ROOT" "$variant" "$QUERY_SET" "$model_dir" "$ebqa_train" "$ebqa_eval" "$out_cfg" <<'PY'
import json
import sys
from pathlib import Path
...
tpl = root / "dapt_eval_package" / "pre_struct" / "ebqa" / f"ebqa_config_noise_{variant}.json"
if not tpl.is_file():
