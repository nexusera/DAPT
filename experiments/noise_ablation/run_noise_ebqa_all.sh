...
run_variant_ebqa() {
  ...
  local ebqa_cfg="${GEN_DIR}/ebqa_config_noise_${variant}.json"
  local ebqa_best_dir="${DAPT_ROOT}/runs/ebqa_noise_${variant}/best"
  local ebqa_pred_qa="${DAPT_ROOT}/runs/ebqa_noise_${variant}_preds.jsonl"
  local ebqa_pred_doc="${DAPT_ROOT}/runs/ebqa_noise_${variant}_doc_preds.jsonl"
  local ebqa_aligned_dir="${DAPT_ROOT}/runs/ebqa_noise_${variant}_aligned"
  ...
  local report_t2="${DAPT_ROOT}/runs/noise_${variant}_report_task2.json"
...
for variant in "${VARIANTS[@]}"; do
  echo "  - ${variant} Task2: ${DAPT_ROOT}/runs/noise_${variant}_report_task2.json"
done
