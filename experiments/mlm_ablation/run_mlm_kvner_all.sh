...
run_variant_kvner() {
  ...
  local kv_cfg="${GEN_DIR}/kv_ner_config_mlm_${variant}.json"
  local kv_best_dir="${DAPT_ROOT}/runs/kv_ner_finetuned_mlm_${variant}/best"
  ...
  local report_t1="${DAPT_ROOT}/runs/mlm_${variant}_report_task1.json"
  local report_t3="${DAPT_ROOT}/runs/mlm_${variant}_report_task3.json"
...
echo "[OK] MLM ablation KV-NER 阶段完成。"
for variant in "${VARIANTS[@]}"; do
  echo "  - ${variant} Task1: ${DAPT_ROOT}/runs/mlm_${variant}_report_task1.json"
  echo "  - ${variant} Task3: ${DAPT_ROOT}/runs/mlm_${variant}_report_task3.json"
done
