...
if ! grep -q -- '"--mlm_masking"' "${DAPT_ROOT}/train_dapt_macbert_staged.py"; then
  echo "[ERR] 当前 train_dapt_macbert_staged.py 不支持 --mlm_masking，请先同步新代码。" >&2
  exit 1
fi
...
run_variant_pretrain() {
  ...
  local gpu="$2"
  local mlm_masking
  mlm_masking="$(variant_to_mlm_masking "$variant")"
  ...
  echo "============================================================"
  echo "[${variant}] PRETRAIN START (CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}, mlm_masking=${mlm_masking}, noise_mode=${NOISE_MODE})"
  echo "============================================================"
  ...
  local master_port=$((MASTER_PORT_BASE + variant_idx))
