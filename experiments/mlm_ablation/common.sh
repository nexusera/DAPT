...
if [[ -n "${MLM_ABLATION_COMMON_SH_LOADED:-}" ]]; then
  return 0
fi
MLM_ABLATION_COMMON_SH_LOADED=1

set -euo pipefail

MLM_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_DAPT_ROOT="/data/ocean/DAPT"
if [[ ! -d "$DEFAULT_DAPT_ROOT" ]]; then
  DEFAULT_DAPT_ROOT="$(cd "${MLM_SCRIPT_DIR}/../.." && pwd)"
fi
...
LOG_DIR="${LOG_DIR:-${DAPT_ROOT}/runs/mlm_ablation_logs}"
GEN_DIR="${GEN_DIR:-${DAPT_ROOT}/experiments/mlm_ablation/generated_configs}"
mkdir -p "$LOG_DIR" "$GEN_DIR" "${DAPT_ROOT}/runs" "${DAPT_ROOT}/data/kv_ner_prepared_comparison"

variant_to_mlm_masking() {
  local variant="$1"
  case "$variant" in
    kvmlm|kv_wwm)
      echo "kv_wwm"
    ...
    plainmlm|token)
      ...
      ;;
    *)
      echo "[ERR] 未知 variant=${variant}，仅支持 kvmlm/plainmlm（或 kv_wwm/token）" >&2
      return 1
      ;;
  esac
}
...
pretrain_output_dir() {
  local variant="$1"
  echo "${DAPT_ROOT}/workspace/output_ablation_mlm_${variant}"
}
...
kv_summary_path() {
  local variant="$1"
  echo "${DAPT_ROOT}/runs/mlm_${variant}_eval_summary.json"
}

kv_aligned_gt() {
  local variant="$1"
  echo "${DAPT_ROOT}/runs/mlm_${variant}_task13_aligned_gt.jsonl"
}

kv_aligned_pred() {
  local variant="$1"
  echo "${DAPT_ROOT}/runs/mlm_${variant}_task13_aligned_preds.jsonl"
}

ebqa_train_jsonl() {
  local variant="$1"
  echo "${DAPT_ROOT}/data/kv_ner_prepared_comparison/ebqa_train_real_mlm_${variant}.jsonl"
}

ebqa_eval_jsonl() {
  local variant="$1"
  echo "${DAPT_ROOT}/data/kv_ner_prepared_comparison/ebqa_eval_real_mlm_${variant}.jsonl"
}

gen_kv_config() {
  local variant="$1"
  local model_dir="$2"
  local out_cfg="${GEN_DIR}/kv_ner_config_mlm_${variant}.json"

  "$PYTHON_BIN" - "$DAPT_ROOT" "$NOISE_MODE" "$variant" "$REAL_TRAIN_JSON" "$REAL_TEST_JSON" "$model_dir" "$out_cfg" <<'PY'
import json
import sys
from pathlib import Path

root = Path(sys.argv[1])
noise_mode = sys.argv[2]
variant = sys.argv[3]
real_train = sys.argv[4]
real_test = sys.argv[5]
model_dir = sys.argv[6]
out_cfg = Path(sys.argv[7])
tpl = root / "dapt_eval_package" / "pre_struct" / "kv_ner" / f"kv_ner_config_noise_{noise_mode}.json"
if not tpl.is_file():
...
kv["train"]["output_dir"] = f"runs/kv_ner_finetuned_mlm_{variant}"
...
gen_ebqa_config() {
  ...
  local ebqa_train="$3"
  local ebqa_eval="$4"
  local out_cfg="${GEN_DIR}/ebqa_config_mlm_${variant}.json"

  "$PYTHON_BIN" - "$DAPT_ROOT" "$NOISE_MODE" "$variant" "$QUERY_SET" "$model_dir" "$ebqa_train" "$ebqa_eval" "$out_cfg" <<'PY'
import json
import sys
from pathlib import Path
...
noise_mode = sys.argv[2]
variant = sys.argv[3]
query_set = sys.argv[4]
model_dir = sys.argv[5]
ebqa_train = sys.argv[6]
ebqa_eval = sys.argv[7]
out_cfg = Path(sys.argv[8])
tpl = root / "dapt_eval_package" / "pre_struct" / "ebqa" / f"ebqa_config_noise_{noise_mode}.json"
if not tpl.is_file():
    raise FileNotFoundError(f"Missing EBQA template: {tpl}")
with tpl.open("r", encoding="utf-8") as f:
    ebqa = json.load(f)
output_dir = root / "runs" / f"ebqa_mlm_{variant}"
