...
run_variant_ebqa() {
  ...
  local ebqa_cfg="${GEN_DIR}/ebqa_config_nsp_ratio_${ratio_name}.json"
  local ebqa_best_dir="${DAPT_ROOT}/runs/ebqa_nsp_ratio_${ratio_name}/best"
  local ebqa_pred_qa="${DAPT_ROOT}/runs/ebqa_nsp_ratio_${ratio_name}_preds.jsonl"
  local ebqa_pred_doc="${DAPT_ROOT}/runs/ebqa_nsp_ratio_${ratio_name}_doc_preds.jsonl"
  local ebqa_aligned_dir="${DAPT_ROOT}/runs/ebqa_nsp_ratio_${ratio_name}_aligned"
  ...
  local report_t2="${DAPT_ROOT}/runs/nsp_ratio_${ratio_name}_report_task2.json"
  ...
  echo "============================================================"
  echo "[${variant}] EBQA START (ratio=${ratio_name}, CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES})"
  echo "============================================================"
...
for variant in "${VARIANTS[@]}"; do
  ratio_name="$(variant_ratio_name "$variant")"
  echo "  - ratio ${ratio_name} Task2: ${DAPT_ROOT}/runs/nsp_ratio_${ratio_name}_report_task2.json"
done
