...
### 1.1 任务范围
- **Task1/Task3（KV-NER）**：按 `pipeline_xiaorong.md` 的训练/推理/评测流程。
- **Task2（EBQA）**：按 `pipeline_task2_xiaorong.md` 的转换/训练/推理/评测流程。
...
### 7.1 先获得各模型预测结果
- Task1/3：沿用 `pipeline_xiaorong.md` 中 `compare_models.py` + 对齐 + scorer 流程
- Task2：沿用 `pipeline_task2_xiaorong.md` 中 `predict_ebqa.py` + 聚合 + 对齐 + scorer 流程
