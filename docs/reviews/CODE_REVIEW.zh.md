...
```
 原始 JSON / OCR（多源）
        │
        ▼                                     scripts/data/extract_and_dedup_json_v2.py （MD5 去重）
 train.txt + 按源拆分文件
        │
        ▼                                     scripts/data/resample_mix.py  +  chunk_long_lines.py
...
6. **论文级文档** — `docs/notes/paper.md`（LLNCS LaTeX）、`docs/pipelines/pipeline_new.md`、`docs/guides/PRETRAINED_MODELS_SUMMARY.md`、`docs/notes/interview_prep.md`、`docs/guides/KV_BERT_预训练与下游推理指南.md`。
...
| N5 | `docs/notes/paper.md` | 匿名块和作者块同时存在，非匿名提交前需清理。<br>**✅ 已修** 移除 `\begin{document}` 后多余的 `\author{Anonymous Authors}` 硬编码行，统一由顶部 `\ifanonymous...\fi` 开关控制；添加注释防止误提交。 |
...
| 分组 | 代表文件 | 状态 |
|------|---------|------|
| 数据摄入 | `scripts/data/extract_and_dedup_json_v2.py`、`scripts/data/resample_mix.py`、`chunk_long_lines.py` | 良好 |
