...
本文档汇总当前实验仓库中 **KV-BERT**（噪声鲁棒、面向半结构化键值抽取的预训练框架）的**数据准备、预训练、推理与评测**流程，并与 `../pipelines/pipeline_new.md`、`../pipelines/pipeline_xiaorong.md`、`../pipelines/pipeline_task2_xiaorong.md` 及 `../notes/paper.md` 中的设定对齐。路径以服务器示例 `/data/ocean/DAPT` 为准，本地开发时可按需替换。
...
- **输入**：标准 Token / Position / Segment 嵌入，外加 **Noise Embedding**：对 OCR 输出的 **7 维物理特征**（置信度统计、标点异常率、断字率、版面对齐等）做建模，与文本表示相加后送入 Encoder（论文公式与 `../notes/paper.md` 中 Methodology 一致）。
- **预训练任务**：
...
| 步骤         | 脚本                             | 说明                                  |
| ---------- | ------------------------------ | ----------------------------------- |
| 提取与 MD5 去重 | `scripts/data/extract_and_dedup_json_v2.py` | 多源 JSON/TXT → 干净文本，如 `train.txt`    |
| 分源重采样（可选）  | `scripts/data/resample_mix.py` | 调整临床/书籍/论文/通用等占比                    |
...
- 脚本逻辑见 `../pipelines/pipeline_new.md` 第 2.1 节内联 Python（`noise_feature_processor`）。

### 2.4 数据集构建（分路，避免噪声错配）

- **OCR 路**（有真实噪声对齐）  
  - 导出文本：`scripts/data/export_ocr_texts.py`  
...
- **非 OCR 路**（书籍/指南/百科等）  
  - 同一脚本构建，**可 `--shuffle_split`**；无 OCR 元信息时，训练时由 collator 填「完美」噪声向量 `[1,1,0,0,0,0,0]`（与 `../pipelines/pipeline_new.md` 一致）。
...
详见 `../pipelines/pipeline_new.md` 第 3 节。示例：
...
消融（MLM 掩码、NSP 负样本比例、noise bucket/linear/mlp/concat_linear）的完整命令同样列于 `../pipelines/pipeline_new.md`。
...
| 任务         | 含义                | 主流程文档                        |
| ---------- | ----------------- | ---------------------------- |
| Task 1 / 3 | KV 结构化抽取（span 级等） | `../pipelines/pipeline_xiaorong.md`       |
| Task 2     | 基于查询的抽取（EBQA 形式）  | `../pipelines/pipeline_task2_xiaorong.md` |
...
批量跑法：`experiments/nsp_ratio_ablation/run_nsp_ratio_kvner_all.sh`（见 `../pipelines/pipeline_xiaorong.md`）。
...
`experiments/nsp_ratio_ablation/run_nsp_ratio_ebqa_all.sh`（见 `../pipelines/pipeline_task2_xiaorong.md`）。
...
| 预训练流程说明        | `../pipelines/pipeline_new.md`                                                                     |
...
| 论文方法与符号        | `../notes/paper.md`                                                                                |


---

## 8. 常见问题（摘自 `../pipelines/pipeline_new.md`）
...
- 本文档由仓库内 `docs/pipelines/pipeline_new.md`、`docs/pipelines/pipeline_xiaorong.md`、`docs/pipelines/pipeline_task2_xiaorong.md` 与 `docs/notes/paper.md` 整理而成；**具体绝对路径、GPU 编号、conda 环境名**请以实际部署为准。
