...
aggregate_qa_preds_to_doc.py（QA 级 → 文档级聚合，--prefer score 取最高置信度 span）
       ↓
preprocess_ebqa_real_h200.py（位于 MedStruct-S-master/utils，下游 text_hash 对齐，将 doc-level 预测与 GT 的 id 对上）
       ↓
scripts/run_medstruct_scorer.py --task_type task2（官方 MedStruct-S scorer wrapper）
```
...
train.txt（每行一篇文档）
        ↓ scripts/data/resample_mix.py（可选，多源配比重采样）
train_resampled.txt
...
数据转换（convert_ebqa.py）→ 微调（train_ebqa.py）→ 推理（predict_ebqa.py）
     ...
     → scripts/run_medstruct_scorer.py（--task_type task2）
```
